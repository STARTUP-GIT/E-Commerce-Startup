...
export const renderBaseTemplate = ({
  ...
  const brandName = marketplaceName || process.env.APP_NAME || 'Marketplace';
  return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
          ...
          <table role="presentation" width="100%" max-width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;background:linear-gradient(135deg,#111827,#1f2937);border:1px solid rgba(255,255,255,0.08);border-radius:24px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
            <tr>
              <td style="background:${BRAND_COLOR};padding:24px 32px;">
                <div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.8);display:flex;align-items:center;gap:8px;">
                  ${logoUrl ? `<img src="${logoUrl}" alt="${brandName}" style="height:20px;max-width:100px;object-fit:contain;" />` : ''}
                  <span>${brandName}</span>
                </div>
            ...
            <tr>
              <td style="padding:0 32px 32px 32px;color:#9ca3af;font-size:12px;line-height:1.7;">
                ${footerNote || `This message was sent by ${brandName}. If you did not request this action, you can safely ignore it.`}
                <div style="margin-top:12px;">© ${new Date().getFullYear()} ${brandName}. All rights reserved.</div>
              </td>
            </tr>
          </table>
        </td>
