...
export function SetupGuard({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col justify-center items-center p-8 space-y-4">
        ...
        <p className="text-xs text-white/40 text-center max-w-xs">
          Could not verify platform status. Please check your connection and try again.
        </p>
        <button
          onClick={() => refetch()}
  ...
  // If the platform is not initialized, ignore any existing cookie/session and force the setup view
  if (!data.initialized) {
    return <SetupPage />;
  }
