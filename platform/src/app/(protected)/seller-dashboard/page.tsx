...
import { Store, Package, DollarSign, BarChart3, RefreshCw, ToggleLeft, ToggleRight, Info } from "lucide-react";
...
const SELLER_CATEGORIES: CategoryGroup[] = [
  {
    name: "Catalog & Product Management",
    description: "Features for managing products, uploads, variants, and store branding.",
    icon: Package,
    keys: ["PRODUCT_UPLOAD", "BULK_UPLOAD", "VARIANTS", "STORE_BANNER", "STORE_LOGO"],
  },
  {
    name: "Finance & Payment Integrations",
    description: "Seller payout configuration, bank accounts, GST, and gateway integrations.",
    icon: DollarSign,
    keys: ["PAYMENTS", "RAZORPAY", "BANK_ACCOUNT", "GST"],
  },
  {
    name: "Operations & Growth",
    description: "Offers, discounts, inventory tracking, and seller analytics.",
    icon: BarChart3,
    keys: ["OFFERS", "COUPONS", "INVENTORY", "ANALYTICS"],
  },
];
...
export default function SellerDashboardPage() {
  ...
  const fetchSellerFeatures = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/platform/feature-flags?application=SELLER");
      if (res.data?.features) {
        setFeatures(res.data.features);
      }
    } catch (err) {
      console.error("Failed to load seller features", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-sm font-medium text-white/50">Loading seller features...</p>
      </div>
    );
  }
  ...
  const knownKeys = SELLER_CATEGORIES.flatMap((c) => c.keys);
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-white tracking-tight">Seller Dashboard Features</h1>
            <p className="text-xs text-white/50 font-medium">
              Enable or disable seller portal capabilities deployed by the engineering team.
            </p>
          </div>
        </div>
        <button
          onClick={fetchSellerFeatures}
      ...
      {/* Deployment Banner */}
      <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-start gap-3 text-xs text-purple-200">
        <Info className="h-5 w-5 shrink-0 text-purple-400 mt-0.5" />
        <div>
          <span className="font-bold block text-purple-300">Backend Auto-Registration</span>
          Seller features are registered directly from code definitions during backend startup. Platform administrators can only toggle whether a deployed feature is enabled.
        </div>
      </div>
          ...
          <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/10 backdrop-blur-md space-y-4">
            <div className="flex items-center gap-2.5 pb-2 border-b border-white/5">
              ...
              <div>
                ...
                <p className="text-[11px] text-white/40">Other seller features registered in code.</p>
              </div>
            </div>
